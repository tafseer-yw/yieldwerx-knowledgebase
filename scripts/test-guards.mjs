...
import { scanContent, secretPathRule } from '../plugins/yieldwerx-knowledgebase/scripts/lib/guards/secrets.mjs';
import { evaluate, writtenText } from '../plugins/yieldwerx-knowledgebase/scripts/guards/write-guard.mjs';
import { emitDecision, mask } from '../plugins/yieldwerx-knowledgebase/scripts/lib/guards/hook-io.mjs';
...
check('a credential in a document is denied and masked', () => {
  ...
  const v = evaluate({ tool_input: { file_path: '/kb/docs/notes.md', content: `token: "${secret}"` } });
...
check('env indirection and placeholders are not credentials', () => {
  for (const line of ['password: "your_password_here"', 'token: process.env.AIO_API_TOKEN', 'api_key: "example-key-value"']) {
...
check('the override is honoured', () => {
  const payload = { tool_input: { file_path: '/kb/docs/x.md', content: 'k = "AKIA2E0ZTRPQHV4XN9WB"' } };
